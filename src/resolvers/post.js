...
export default {
  ...
  Post : {
    ...
    is_liked: async({
      ...
      const postLike = (await knex('post_likes').where({post_id: id}).andWhere({user_id: user.id})).length === 1
  ...
  Mutation : {
    ...
    updatePost: async(_, {
      ...
      try {
        ...
        return await Post
          .query()
          .patchAndFetchById(id, {title, content, image_url})
      } catch (err) {
        throw err
      }
    },
    ...
    likePost: async(_, {
      ...
      try {
        ...
        const alreadyLiked = (await knex('post_likes').where({post_id: id}).andWhere({user_id: user.id})).length === 1
